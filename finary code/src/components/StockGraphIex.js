...
// util
const formatDateMinute = timeFormat("%H:%M %p");
...
// accessors
const xStock = (d) => new Date(d.date + (d.minute ? " " + d.minute : " EST"));
const yStock = (d) => d.close;
const bisectDate = bisector(
  (d) => new Date(d.date + (d.minute ? " " + d.minute : " EST"))
).left;

class StockGraphIex extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const yScale = scaleLinear({
      range: [yMax, 0],
      domain: [
        min(stock, yStock) - yMax / 1000,
        max(stock, yStock) + yMax / 1000,
      ],
      nice: true,
    });
          ...
          {tooltipData && (
            <g>
              ...
              <text x={tooltipLeft - 20} y={yMax} class="small">
                {this.props.timeFrame === "1D"
                  ? formatDateMinute(xStock(tooltipData))
...
export default withParentSize(withTooltip(StockGraphIex));
